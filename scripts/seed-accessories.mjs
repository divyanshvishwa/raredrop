...
async function seed() {
  console.log("Seeding 12 accessory products...\n");
  ...
  if (error) {
    console.error("Error inserting accessories:", error.message);
    process.exit(1);
  }

  console.log(`Successfully inserted ${data.length} accessory products:\n`);
  data.forEach((p) => {
    console.log(`  - ${p.name} — Rs.${p.price} (${p.type})`);
  });
  console.log("\nDone!");
}
